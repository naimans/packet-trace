...
def udpfunction():
    ...
    if udp.len != udp_length:
        udp_output[ip_pair] = {"length": udp.len}
        ot.write_packet(pkt)
...
startTime = time.time() #Calculate the time of the trace parsing
ot = plt.output_trace(f'pcapfile:sniffed_'+str(sys.argv[1]).rsplit('.', )[0]+'.pcap') #store all the connections with intersting header field
ot_ip = plt.output_trace(f'pcapfile:interesting_ip_'+str(sys.argv[1]).rsplit('.', )[0]+'.pcap') #ip conncetions whose version is not 4/6
ot_ip.start_output()
ot.start_output()
...
for pkt in t:
    ...
    if ip6:
        ipv6function()
    if tcp: #for both ipv4 and v6
        tcpfunction()    
    if icmp:
        icmpfunction()

#generating a csv formate output with all unusal values
...
#get all the packtes count

count_dict = {'ipv4': ipv4_pkt_count, 'ipv6': ipv6_pkt_count, 'tcp': tcp_pkt_count, 'udp': udp_pkt_count, 'icmp': icmp_pkt_count}
field_names = ['ipv4','ipv6', 'tcp', 'udp', 'icmp']

with open(r'packets_count_'+str(sys.argv[1]).rsplit('.',)[0]+'.csv', 'w') as csvfile:  
